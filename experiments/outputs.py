...
def crepe_outputs(outputs_path):
    ...
    for tid in mdb_pitch_index:
        output_dict[tid] = os.path.join(
            outputs_path, "crepe", "medleydb-pitch", "{}.f0.csv".format(tid)
        )
    return output_dict
...
def deepsalience_outputs(outputs_path):
    ...
    for tid in ikala_index:
        output_dict[tid] = os.path.join(
            outputs_path, "deepsalience", "ikala", "{}_melody2_singlef0.csv".format(tid)
        )
...
def melodia_outputs(outputs_path):
    ...
    for tid in ikala_index:
        output_dict[tid] = os.path.join(
            outputs_path, "melodia", "ikala", "{}.wav.f0.csv".format(tid)
        )
    mdb_mel_index = mirdata.medleydb_melody.track_ids()
    for tid in mdb_mel_index:
        output_dict[tid] = os.path.join(
            outputs_path, "melodia", "medleydb-melody", "{}_MIX.wav.f0.csv".format(tid)
        )
    orchset_index = mirdata.orchset.track_ids()
    for tid in orchset_index:
        output_dict[tid] = os.path.join(
            outputs_path, "melodia", "orchset", "{}.wav.f0.csv".format(tid)
        )
    return output_dict
...
def load_pyin(fpath):
    ...
    times = np.arange(0, max_time, 256.0 / 44100.0)
