...
def ikala(track_id, source_sep=False):
    ...
    if source_sep:
        confidence_fpath = os.path.join(
            "../experiments/new_conf_2x/separation",
            "iKala", "{}_lead_VUIMM.wav.conf.csv".format(track_id))
    else:
        confidence_fpath = (
            "../experiments/new_conf_2x/stems/iKala/{}.wav.conf.csv".format(track_id))
...
def medleydb_pitch(track_id):
    ...
    confidence_fpath = (
        "../experiments/new_conf_2x/stems/MedleyDB-Pitch/{}.wav.conf.csv".format(track_id))
