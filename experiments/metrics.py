...
def interp_with_zeros(x, y, kind, x_new):
    ...
    smooth_resampled = scipy.interpolate.interp1d(
        x, y, kind, fill_value=0.0, bounds_error=False
    )(x_new)

    # If there's a zero, resample 0's everywhere until the next nonzero value
    resampler_mask = scipy.interpolate.interp1d(
        x, y, "previous", fill_value=0.0, bounds_error=False
    )(x_new) * scipy.interpolate.interp1d(
        x, y, "next", fill_value=0.0, bounds_error=False
    )(
        x_new
    )
